...
void applyRotationCorrection(Quaternion& q) {
  // Rotation corrective de 90° autour de Z (convertie en quaternion)
  Quaternion correction(0.7071, 0, 0.7071, 0); // cos(45°), x=0, y=sin(45°), z=0

  // Appliquer la rotation corrective
  Quaternion corrected = correction.getProduct(q);
  corrected.normalize();

  // Mettre à jour le quaternion avec les valeurs corrigées
  q.w = corrected.w;
  q.x = corrected.x;
  q.y = corrected.y;
  q.z = corrected.z;
